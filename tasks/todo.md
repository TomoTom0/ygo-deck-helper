...
#### 1.1 基本設計
- [ ] `src/stores/settings.ts` 作成
  - [ ] 画像サイズ設定（small/medium/large/llarge）
  - [ ] テーマ設定（dark/light/system）
  - [ ] 言語設定（auto/ja/en/ko/ae/cn/de/fr/it/es/pt）
  - [ ] その他設定項目の定義
...
### 2. USP（URL State Parameters）実装（優先度：最高）
...
- [ ] パラメータ一覧の定義
  ...
  - `sort`: ソート順
  - `tab`: カードタブ（info/qa/related/products）
  - `size`: 画像サイズ（small/medium/large/llarge）
  - `theme`: テーマ（dark/light/system）
  - `lang`: 言語（auto/ja/en/ko/...）
  - その他フィルター条件
- [ ] パラメータのエンコード/デコードルール
...
#### 3.2 コンポーネント修正
- [ ] `src/components/DeckCard.vue` 修正
- [ ] `src/components/CardList.vue` 修正
- [ ] `src/components/CardInfo.vue` 修正
- [ ] その他カード表示コンポーネント
...
#### 4.1 テーマ定義
- [ ] `src/styles/themes.ts` 作成
  - ダークテーマの色定義
  - ライトテーマの色定義
  - CSS変数マッピング
- [ ] システムテーマ検出（`prefers-color-scheme`）

#### 4.2 テーマ適用ロジック
- [ ] `src/utils/theme-manager.ts` 作成
- [ ] テーマ切り替え関数
- [ ] システムテーマ変更の監視
